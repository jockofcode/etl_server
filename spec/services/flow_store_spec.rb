...
RSpec.describe FlowStore do
  ...
  describe ".all" do
    ...
    it "returns summaries for each flow file" do
      ...
      expect(result.first[:id]).to eq("my_flow")
  ...
  describe ".create" do
    it "creates a new flow file" do
      FlowStore.create("new_flow", valid_flow)
      expect(File.exist?(File.join(tmp_dir, "new_flow.yml"))).to be true
    end

    it "raises FlowAlreadyExists if id is taken" do
      FlowStore.create("dup_flow", valid_flow)
      expect { FlowStore.create("dup_flow", valid_flow) }.to raise_error(FlowStore::FlowAlreadyExists)
    end
    ...
    it "raises InvalidFlowData when START_NODE is missing" do
      expect { FlowStore.create("no_start", { "step1" => {} }) }.to raise_error(FlowStore::InvalidFlowData)
    end

    it "raises InvalidFlowData when START_NODE has no name" do
      bad = { "START_NODE" => { "description" => "oops" } }
      expect { FlowStore.create("bad_start", bad) }.to raise_error(FlowStore::InvalidFlowData)
    end
  end
  ...
  describe ".copy" do
    it "duplicates the flow under a new id" do
      ...
      FlowStore.copy("original", "copy_of_original")
      expect(FlowStore.find("copy_of_original")["START_NODE"]["name"]).to eq("My Flow")
    end
